...
Backend service required for downloading functionality.`,
  ...
  const handleExportBackup = async () => {
    try {
      Alert.alert('Feature Coming Soon', 'Export backup functionality will be available in a future update');
    } catch (error) {
  ...
  const handleExportURLs = async () => {
    try {
      Alert.alert('Feature Coming Soon', 'Export URLs functionality will be available in a future update');
    } catch (error) {
      ...
      items: [
        {
          ...
          subtitle: `${localSettings.downloadFormat === 'audio' ? 'Audio Only (MP3)' : 'Full Video (MP4)'} - Applies to YouTube, TikTok, Instagram Reels`,
          hasArrow: true,
